...
public class con
{
	...
	public static boolean smartMove(char[][] board,boolean victory,int targetMatches)
	{
		...
		for(int w = 0; w < board[0].length; w++)
		{
			...
			if(checkWinner(board,!victory,targetMatches,row,i) && check)
			{
				if(chip == 'P' && row >= 0)
				{
					board[row][i] = 'X';
	...
	public static void main(String[] args)
	{
		...
		while(!playerWon && !computerWon)
		{
			...
			/*if(changed && which == 0)
			{
				System.out.println("Got 4 in a row!");
				which++;
			}*/
			...
			if(!changed) 
			{
				...
				dummyMove(board); //No moves smart moves avaiable? Randomly place one.
			}
